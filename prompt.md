## 项目需求描述（prompt）
<!-- ! 自己的需求描述不够专业，考虑的不够周到 ，记录完自己的需求和想法后，不能直接让AI去实现，应该先让AI完善这个想法，让这个想法更具体，更全面。之后再问AI，这个想法有哪些需要补充，有没有哪些可能出现的问题和漏洞，”我应该如何优化这个需求描述，才能让AI在执行时不会出现结果的差错，不会让AI觉得有歧义或者误会我的描述“，完善之后再让交给AI实现。如果想法需要多点灵感，问AI -->

<!-- ! 不要闷着头就自己想什么什么然后交给AI，网上有很多高级易用的UI模板和组件轮子，很有风格。下面是资源汇总：
! 【程序员没博客？做贼酷炫个人网站的5个技巧！】 https://www.bilibili.com/video/BV1q81XYAEuW/?share_source=copy_web&vd_source=8f3a13fe3c540f52bc65310cd808ac9a
!
!
!
! -->

> 本项目旨在开发一个个人成长记录型博客网站
...
1. 在管理员模式下的时间轴页面里。点击新增节点按钮，可进入节点编辑页面(还包含节点删除,节点保存按钮)，可编辑节点标题、内容、时间、标签等信息.编辑后点击保存按钮，节点信息将保存至数据库并自动更新时间轴;若不点击保存按钮,则退出编辑页面时将忽略此次新增操作,不把新增信息存至数据库.
   1. 点击编辑节点按钮后,再点击某个已存在的节点,会弹出对应的编辑页面，允许修改节点标题、内容、时间、标签等信息.编辑后点击保存按钮，节点信息将保存至数据库并自动更新时间轴;若不点击保存按钮,则退出编辑页面时将忽略此次修改操作,不把修改信息存至数据库.
2. 增加删除节点按钮，点击删除节点按钮后，再点击某个已存在的节点，会弹出确认删除提示，确认后将删除该节点，并删除数据库中的该节点信息，并自动更新时间轴。
3. 添加网页背景音乐，循环播放。
4. 节点内容支持插入图片、音频、视频等多媒体内容，可自动压缩、转码，确保内容高效加载与兼容性。
5. 在管理员模式下的时间轴页面中，在新建节点和节点编辑的编辑页面中，增加多媒体内容功能，可上传图片、音频、视频等多媒体内容，点击保存后，后台自动压缩、转码，并保存至数据库；节点的详情信息页面中，支持多媒体内容的展示与播放。
<!-- todo:  -->
...
新增节点时：
{上传的多媒体文件立即上传到服务器，生成预览。
如果点击“取消”或关闭窗口，所有本次会话中新上传的文件（包括图片原图和缩略图、音频、视频）都要被删除，数据库中也不应有这些文件的记录。
如果点击“保存”，只保留当前页面预览列表中的文件，未被引用的临时文件要清理。}

编辑已有节点时：
{上传新文件时，逻辑同上：未保存时新文件要清理，保存时只保留被引用的文件。
未被引用的临时文件（即本次编辑中新上传但未被保留在预览列表中的文件）要被彻底清理（包括服务器文件和数据库记录）。
如果点击“取消”或关闭窗口，则本次编辑中新上传的所有文件都要被清理，节点和数据库的文件内容应和编辑前完全一致（没有新增文件，也没有减少文件）。换句话说，未保存时,编辑前的节点内容和文件应完整保留，不能有任何变动。}

文件清理细节：
{图片需删除原图和缩略图。
音频、视频只需删除原文件。
文件的唯一性可通过文件名或上传时生成的唯一 ID 追踪。}


注意:如果上传的是音频或视频文件B,则upload文件夹里仅包含原文件B,所以你要区分文件A是否为图片,如果是那么就删除A的原文件和缩略图,否则只删除A的原文件.

bug:节点保存文件过多时,详情界面不能下滑.

两节点间距与两节点日期时间差值成正比，比例系数可以通过 鼠标滚轮的滑动 缩放（不需要有UI控件）。。

网页增加天气效果和自然场景更迭，落叶/海洋/雨天/雪天/烈日/夜晚/平原/山地/空中/云层/大气层/星空/太阳系/。。 随着时间轴往前，模拟从起点穿过各个地点，，再从地面到空中到。。。
